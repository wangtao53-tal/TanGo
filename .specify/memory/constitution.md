<!--
Sync Impact Report:
Version: 2.1.0 → 2.2.0 (新增对话Agent技术规范和MVP优先原则)
新增原则:
  - 原则八：对话Agent技术规范
修改原则:
  - 原则二：K12 教育游戏化设计规范（扩展教育目标描述）
  - 原则三：可发布应用规范（新增MVP优先要求）
  - 原则五：AI优先（模型优先）规范（强调Eino框架实现）
模板更新状态:
  - ✅ .specify/templates/plan-template.md (已更新 Constitution Check 部分，包含原则八)
...
**理由**：确保产品符合目标用户群体的特点，实现寓教于乐的教育目标，提供丰富的课外学习支持。
...
实现的应用必须是可以使用、可以发布的生产级应用。遵循MVP优先原则，快速实现页面上所有功能，先确保页面调通，保证功能的完整可用性。

**理由**：确保项目具有实际价值，能够真正服务于目标用户。MVP优先原则确保快速迭代和功能完整性。
...
对话页发送文本消息和AI对话Agent调用必须基于Eino Graph实现，支持联网获取信息、图文混排输出、SSE流式输出和打字机效果。

**理由**：确保对话功能的完整性和用户体验的流畅性，通过Eino Graph实现灵活的AI工作流编排，支持丰富的交互形式。
