<!--
Sync Impact Report:
Version: 2.5.0 → 2.6.0 (前端中文优先规范强化)
新增治理规则:
  - 无
修改原则:
  - 原则四：多语言和年级设置规范 - 强化中文优先要求，明确所有页面中文优先
模板更新状态:
  - ✅ .specify/memory/constitution.md (已更新原则四，强化中文优先要求)
  - ✅ .specify/templates/plan-template.md (已更新原则四描述，强调中文优先)
  - ✅ .specify/templates/spec-template.md (模板结构保持，实际生成内容需遵循中文优先规范)
  - ✅ .specify/templates/tasks-template.md (模板结构保持英文，实际生成内容需遵循中文规范)
-->
...
**中文优先原则**：前端项目必须遵循中文优先规范，所有页面默认显示中文，支持中英文两种语言设置。中文是主要语言，英文作为可选语言提供。

**理由**：确保中文用户获得最佳体验，同时满足不同用户的语言需求。根据年级提供适配的内容难度，确保教育内容的针对性。

**具体要求**：
- **中文优先（前端项目）**：
  - **所有页面默认显示中文**：前端所有页面、组件、交互元素默认必须使用中文显示
  - **中文是主要语言**：中文翻译必须完整、准确，覆盖所有页面和功能
  - **英文作为可选语言**：英文翻译作为补充，用户可选择切换
  - **页面级中文优先**：每个页面、每个组件都必须优先考虑中文显示，确保中文内容完整且准确
  - **国际化实现**：使用i18n框架（如react-i18next）实现多语言支持，但中文必须作为默认语言和主要语言
- **语言设置**：
  - 系统默认语言为中文
  - 支持用户切换中英文界面
  - 所有Web界面默认显示中文
...
所有AI相关功能（图片识别、文字识别、继续追问等）都统一通过后端调用模型处理，支持流式返回文字和图片内容。**AI Model First**：模型调用优先，Agent eino框架优先。对话页面必须与AI交互，使用真正的模型，而不是Mock数据。

**理由**：确保AI能力的一致性、可维护性和可扩展性，提供流畅的实时交互体验。Eino框架提供统一的AI模型调用接口和Graph编排能力。真实模型交互确保用户获得有价值的AI响应，提升产品实用性和用户体验。

**具体要求**：
- **模型调用优先**：
  - 所有AI功能必须优先使用真实模型调用，禁止在生产环境使用Mock数据
  - 对话页面必须与AI模型交互，使用真正的模型生成响应
  - Agent eino框架优先：所有AI能力必须通过Eino框架实现
  - 只有在模型调用失败且无法恢复时，才允许降级处理（但必须记录错误日志）
- **统一后端调用**：
  - 所有AI模型调用必须通过后端API实现
  - 前端不直接调用AI模型服务
...
- **Eino框架实现**：
  ...
  - Agent必须基于Eino Graph实现，支持灵活的节点编排
- **Mock数据配置**：
  - 可以有一个配置文件，设置是否使用模型调用，还是Mock数据
  - Mock数据仅允许用于开发/测试环境，禁止在生产环境使用
  - 配置项应明确标识环境类型（development/test/production）
  - 生产环境必须强制使用真实模型，配置项无效
  - Mock数据必须明确标识，便于区分和调试
- **流式返回支持**：
...
对话页发送文本消息和AI对话Agent调用必须基于Eino Graph实现，支持联网获取信息、图文混排输出、SSE流式输出和打字机效果。流式消息必须实时渲染，支持Markdown格式展示。对话页的语音输入和图片上传必须支持Agent模型流式返回内容，禁止使用Mock数据。

**理由**：确保对话功能的完整性和用户体验的流畅性，通过Eino Graph实现灵活的AI工作流编排，支持丰富的交互形式。实时渲染确保用户能够及时看到AI响应，Markdown支持提升内容展示质量。真实模型交互确保用户获得有价值的AI响应，提升产品实用性。
...
- **真实模型交互**：
  - 对话页面必须与AI模型交互，使用真正的模型生成响应
  - 对话页-语音输入后，必须支持Agent模型流式返回内容，禁止使用Mock数据
  - 对话页-上传图片后，必须支持Agent模型流式返回内容，禁止使用Mock数据
  - 所有对话交互必须通过Eino Graph调用真实模型，确保响应质量和准确性
  - 只有在模型调用失败且无法恢复时，才允许降级处理（但必须记录错误日志）
- **联网能力支持**：
...
- **技术实现细节**：
  ...
  - 支持对话上下文的维护和多轮对话
  - 错误处理和降级策略必须完善
  - 语音输入和图片上传必须通过Eino Graph调用真实模型，实现流式返回
...
### 分支管理规范 (NON-NEGOTIABLE)

当前项目处于MVP版本阶段，所有本地开发分支必须统一使用 `dev-mvp-20251218` 分支，直到第一个正式版本推出。

**理由**：MVP阶段需要快速迭代和功能整合，统一分支管理简化开发流程，避免分支过多导致的合并冲突和管理复杂度。在MVP阶段，所有功能开发应在同一分支进行，便于集成测试和快速发布。

**具体要求**：
- **MVP阶段分支策略**：
  - 所有本地开发工作必须在 `dev-mvp-20251218` 分支进行
  - 禁止为每个功能创建独立分支（禁止"一个功能一个分支"的策略）
  - 所有功能开发、bug修复、优化都在同一分支进行
  - 保持 `main` 分支稳定，仅用于发布
- **分支命名**：
  - MVP阶段统一使用：`dev-mvp-20251218`
  - 禁止使用功能分支命名（如 `feat-xxx`、`fix-xxx` 等）
  - 禁止使用spec编号作为分支名（如 `dev-001-xxx`）
- **版本发布后**：
  - 第一个正式版本发布后，可以恢复功能分支策略
  - 版本发布后的分支策略将在后续版本中定义
- **提交规范**：
  - 提交信息必须清晰描述变更内容
  - 使用规范的commit message格式（参考项目Git提交规范）
  - 每次提交应该是原子性的，便于代码审查和回滚
...
- **v2.6.0** (2025-12-22)：前端中文优先规范强化
  - 更新原则四：多语言和年级设置规范，强化中文优先要求
  - 明确所有页面默认显示中文，中文是主要语言
  - 明确页面级中文优先，每个页面、每个组件都必须优先考虑中文显示
  - 明确英文作为可选语言提供，用户可选择切换
  - 强调前端项目必须遵循中文优先规范，确保中文用户获得最佳体验
- **v2.5.0** (2025-12-22)：MVP版本分支管理规范
  - 新增分支管理规范：MVP版本统一使用dev-mvp-20251218分支
  - 明确禁止一个功能一个分支的策略
  - 明确MVP阶段所有开发工作在同一分支进行
  - 明确版本发布后可恢复功能分支策略
- **v2.4.0** (2025-12-21)：模型调用优先、真实模型交互、Mock数据配置规范
  - 更新原则五：AI优先（模型优先）规范，明确模型调用优先、Agent eino框架优先
  - 更新原则五：明确对话页面必须使用真实模型，而不是Mock数据
  - 更新原则五：新增Mock数据配置规范，Mock数据仅允许用于开发/测试环境
  - 更新原则八：对话Agent技术规范，明确语音输入和图片上传必须支持Agent模型流式返回
  - 明确禁止在生产环境使用Mock数据，对话页面必须与AI模型交互
  - 明确语音输入和图片上传必须通过Eino Graph调用真实模型，实现流式返回
