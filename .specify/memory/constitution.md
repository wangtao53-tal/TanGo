<!--
Sync Impact Report:
Version: 2.3.0 → 2.4.0 (模型调用优先、真实模型交互、Mock数据配置规范)
修改原则:
  - 原则五：AI优先（模型优先）规范（明确模型调用优先、Agent eino框架优先、对话页面必须使用真实模型、Mock数据仅用于开发/测试环境）
  - 原则八：对话Agent技术规范（明确语音输入和图片上传必须支持Agent模型流式返回，禁止使用Mock数据）
模板更新状态:
