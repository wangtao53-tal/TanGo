...
export const LittleStar: React.FC<LittleStarProps> = ({ message, className = '' }) => {
  ...
  return (
    <div className={`fixed bottom-20 sm:bottom-6 left-1/2 -translate-x-1/2 w-full max-w-[600px] px-4 z-20 pointer-events-none ${className}`}>
      <div className="flex items-end gap-4 pointer-events-auto animate-float" style={{ animationDelay: '1s' }}>
        <div className="relative shrink-0">
          ...
          <div className="relative size-14 sm:size-20 rounded-full border-4 border-peach-pink bg-white overflow-hidden shadow-lg">
        ...
        <div className="flex flex-col gap-1 mb-2 sm:mb-4">
          <p className="text-text-sub text-xs sm:text-sm font-bold ml-2 sm:ml-4">{t('littleStar.name')}</p>
          <div className="relative bg-white text-text-main px-3 sm:px-6 py-2 sm:py-4 rounded-2xl rounded-bl-none shadow-[0_10px_30px_-5px_rgba(0,0,0,0.1)] border-2 border-peach-pink/50">
            <p className="text-xs sm:text-base md:text-lg font-bold leading-normal">{message}</p>
